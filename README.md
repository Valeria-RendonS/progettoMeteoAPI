...
- [Meteo Frogger: Come Giocare](#-meteo-frogger-come-giocare)
...
| File | Descrizione |
| :--- | :--- |
| `index.html` | **Landing Page.** Intro animata con effetti pioggia e mascotte. |
| `meteo.html` | **Hub Principale.** Selezione località, visualizzazione scena dinamica e previsioni. |
| `meteoAPI.js` | **Core Logic.** Gestisce le chiamate API, la logica delle stagioni e il salvataggio dati. |
| `pagina.html` | **Dettagli.** Grafici approfonditi e dati orari. |
| `frogger.html` | **Il Gioco.** Motore di gioco arcade basato su Canvas con temi meteo adattivi. |
...
### Regole
1.  Attraversa la strada evitando le auto (i camion sono lenti, le auto sportive veloci!).
2.  Attraversa il fiume saltando sui tronchi (non cadere in acqua!).
3.  Raggiungi il prato opposto per segnare punti e resettare la posizione.
4.  **Attenzione:** Ogni 100 punti la velocità del gioco aumenta!
...
* **[Open-Meteo](https://open-meteo.com/)**: Per i dati meteorologici, geocoding e previsioni (Nessuna API Key richiesta).
* **[CountriesNow](https://countriesnow.space/)**: Per il database JSON di nazioni e città.
